...
import { ListUser, verfikasiUser } from "../../services/verifier.api";

const VerifierUsers = ({ navigate, currentPage = "users" }) => {
  ...
  const handleVerification = async () => {
    ...
    try {
      setVerifying(true);
      await verfikasiUser(selectedUser.id);

      setUsers((prevUsers) => prevUsers.map((user) => (user.id === selectedUser.id ? { ...user, verifikasi: "Terverifikasi" } : user)));

      setShowVerificationModal(false);
      setSelectedUser(null);
      showSuccess("User berhasil diverifikasi!", { duration: 4000 });
    } catch (error) {
      showWarning("Gagal memverifikasi user.", { duration: 4000 });
    } finally {
      setVerifying(false);
    }
  };
                          ...
                          <tr key={userData.id || index} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              <div className="flex items-center justify-end space-x-2">
                                {userData.verifikasi !== "Terverifikasi" ? (
                                  <button onClick={() => openVerificationModal(userData)} className="text-green-600 hover:text-green-800 p-1 tooltip" title="Verifikasi User">
        ...
        {showVerificationModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
              <div className="p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Konfirmasi Verifikasi User</h3>
                ...
                <div className="mb-6">
                  <p className="text-gray-700">Apakah Anda yakin ingin memverifikasi user ini? Setelah diverifikasi, user akan dapat mengakses semua fitur yang tersedia.</p>
                </div>

                <div className="flex space-x-3">
                  <button onClick={closeVerificationModal} disabled={verifying} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 disabled:opacity-50">
                    Batal
                  </button>
                  <button onClick={handleVerification} disabled={verifying} className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
